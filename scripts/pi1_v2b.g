...
quick("R+S1", [R0,Ax,Ay,Ar,As,Bx,By,Br,Bs,S1(y*s^-1,1)]);
...
for trial in [1..120] do
  ...
  res := quick(Concatenation("C#",String(trial)),
     [R0,Ax,Ar,Bx,Br,Byc,Bsc,Ayc,Asc,S1(y*s^-1,e1),S2s(e2)]);
  cnt.(res) := cnt.(res) + 1;
od;
