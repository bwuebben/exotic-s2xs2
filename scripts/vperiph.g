...
#
# These are NECESSARY conditions for the six input words to be correct boundary
# classes.  The relation group used below is only known to surject onto the
# complement group.  Consequently, either trivial or nontrivial images in its
# computable quotients are diagnostics, not proofs about the actual peripheral
# classes.  The power controls test only that this diagnostic is non-vacuous.
#
...
# to 6, of the augmented relation group, in all 8 correction-sign
...
# words to test, as functions of e5 (index, name, target word?)
mkWords := function(e5)
  return [
        ...
        if not IsOne(img) then
          if w[3] then
            Print("   p=", p, " cl=", cl, " |Q|=", Size(Q),
                  ": diagnostic target survives: ", w[1], "\n");
            anyMustFail := true;
          else
            Print("   p=", p, " cl=", cl, " |Q|=", Size(Q),
...
for T in [ [SymmetricGroup(3),"S3"], [AlternatingGroup(4),"A4"],
  ...
  for hom in q[2] do
    for w in words do
      ...
      if not IsOne(img) then
        if w[3] then
          Print("    diagnostic target survives in ", T[2], ": ", w[1], "\n");
          anyMustFail := true;
        else
...
Print("\n=== V-PERIPH SUMMARY ===\n");
if anyMustFail then
  Print("*** A DIAGNOSTIC COMMUTATOR SURVIVED. INSPECT THE INPUT WORDS.\n");
else
  Print("All six target commutators trivial in every computed quotient,\n");
...
Print("error class; this battery is diagnostic rather than a proof.)\n");
QUIT_GAP(0);
