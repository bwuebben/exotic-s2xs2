...
# Same target/control words as vperiph.g, evaluated in the permutation
# representations on cosets of ALL subgroups of index <= 5 of the augmented
# relation group (richer than epimorphisms onto fixed targets).  Because this
# group only surjects onto the complement group, the outcomes are diagnostic.
...
for H in li do
  ...
  for w in words do
    if not IsOne(Image(hom, wg(w[2]))) then
      if w[3] then
        Print("diagnostic target survives (index ", Index(G,H), "): ", w[1], "\n");
        mustFail := true;
      else
  ...
  od;
od;
Print("=== vperiph2 summary: surviving targets: ", mustFail,
